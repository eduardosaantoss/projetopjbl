...
"""
        ...
        for v in listar_visitas(cliente_id):
            visitas_tree.insert("", "end", values=(formatar_data_brasileira(v['data_visita']), v['tipo_visita'], v['resumo']))
        ...
        for p in listar_pendencias(cliente_id):
            pend_tree.insert("", "end", values=(p['descricao'], formatar_data_brasileira(p['prazo']), p['prioridade'], p['status']))
        ...
        for con in listar_contratos(cliente_id):
            cont_tree.insert("", "end", values=(con['tipo_contrato'], f"R$ {con['valor']:.2f}", formatar_data_brasileira(con['data_fim'])))
        ...
        for v in listar_visitas(cliente_id):
            f = ttk.LabelFrame(scrollable_frame, text=f"Visita em {formatar_data_brasileira(v['data_visita'])}")
