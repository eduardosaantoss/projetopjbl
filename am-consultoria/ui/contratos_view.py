...
from utils import formatar_data_brasileira, formatar_data_iso, aplicar_mascara_data
...
class ContratosView(ttk.Frame):
    ...
    def atualizar_tabela(self):
        ...
        for c in listar_contratos():
            self.tree.insert("", "end", values=(c['id'], c['cliente_nome'], c['tipo_contrato'], f"R$ {c['valor']:.2f}", formatar_data_brasileira(c['data_inicio']), formatar_data_brasileira(c['data_fim'])))
    ...
    def abrir_cadastro(self):
        ...
        ttk.Label(frame, text="Data Início (dd/mm/aaaa)").pack(anchor="w")
        entry_ini = ttk.Entry(frame)
        aplicar_mascara_data(entry_ini)
        entry_ini.pack(fill="x", pady=(0, 10))

        ttk.Label(frame, text="Data Fim (dd/mm/aaaa)*").pack(anchor="w")
        entry_fim = ttk.Entry(frame)
        aplicar_mascara_data(entry_fim)
        ...
        def salvar():
            try:
                ...
                data_ini_iso = formatar_data_iso(entry_ini.get())
                data_fim_iso = formatar_data_iso(entry_fim.get())
