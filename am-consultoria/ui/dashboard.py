...
class DashboardView(ttk.Frame):
    ...
    def create_card(self, parent, title, value, row, col, colspan=1):
        # Card minimalista com bordas suaves
    ...
    def get_alerts(self):
        ...
        # 1. Clientes sem visita há mais de 30 dias
        cursor.execute("""
            SELECT c.nome 
            FROM clientes c
            LEFT JOIN (
        ...
        # 3. Contratos vencendo em 7 dias
        cursor.execute("""
        ...
        for con in vencendo:
            alerts.append(f"Contrato de '{con['nome']}' vence em breve ({con['data_fim']}).")
